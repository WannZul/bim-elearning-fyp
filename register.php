...
<div class="auth-layout">
    <section class="auth-showcase" aria-label="<?= e(t('auth.intro_label')) ?>">
        ...
        <div class="auth-story">
            <span class="eyebrow"><?= e(t('auth.register.eyebrow')) ?></span><h1><?= e(t('auth.register.headline_before')) ?> <span><?= e(t('auth.register.headline_emphasis')) ?></span></h1><p><?= e(t('auth.register.story')) ?></p>
            <div class="auth-points"><span class="auth-point"><i class="bi bi-check2-circle"></i> <?= e(t('auth.register.point_free')) ?></span><span class="auth-point"><i class="bi bi-shield-check"></i> <?= e(t('auth.register.point_safe')) ?></span><span class="auth-point"><i class="bi bi-graph-up-arrow"></i> <?= e(t('auth.register.point_track')) ?></span></div>
        </div>
    ...
    <section class="auth-panel">
        <div class="auth-form-wrap">
            ...
            <span class="eyebrow"><?= e(t('auth.register.join')) ?></span><h2><?= e(t('auth.register.heading')) ?></h2><p><?= e(t('auth.register.subheading')) ?></p>
            ...
            <form method="POST" action="register.php" data-submit-loading>
                ...
                <div class="form-group-custom"><label for="password"><?= e(t('auth.password')) ?></label><div class="input-shell"><i class="bi bi-lock"></i><input class="input-control-custom" id="password" name="password" type="password" placeholder="<?= e(t('auth.register.password_placeholder')) ?>" minlength="8" autocomplete="new-password" required><button class="password-toggle" type="button" data-password-toggle="password" aria-label="<?= e(t('common.show_password')) ?>"><i class="bi bi-eye"></i></button></div><small class="form-hint"><?= e(t('auth.register.password_hint')) ?></small></div>
                <label class="form-check-custom"><input type="checkbox" required><span><?= e(t('auth.register.consent')) ?></span></label>
                <button class="btn-primary-custom btn-wide" type="submit"><?= e(t('auth.register.submit')) ?> <i class="bi bi-arrow-right"></i></button>
            </form>
            <p class="auth-switch"><?= e(t('auth.register.has_account')) ?> <a href="login.php"><?= e(t('auth.register.login')) ?></a></p>
            <div class="auth-trust"><i class="bi bi-shield-check"></i> <?= e(t('auth.register.trust')) ?></div>
        </div>
    </section>
</div>
