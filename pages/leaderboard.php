...
<div class="leaderboard-content"><div class="container-wide">
    <nav class="ranking-type-tabs surface-card" aria-label="<?= e(t('leaderboard.tabs_label')) ?>" data-reveal><?php foreach ($themes as $type => $theme): ?><a class="ranking-type-tab <?= $type === $selectedType ? 'active' : '' ?>" href="leaderboard.php?type=<?= urlencode($type) ?>" aria-current="<?= $type === $selectedType ? 'page' : 'false' ?>"><span class="icon-tile <?= e($theme['accent']) ?>"><i class="bi <?= e($theme['icon']) ?>"></i></span><span><small><?= e(t($theme['mode'] === 'practical' ? 'leaderboard.mode_practical' : 'leaderboard.mode_theory')) ?></small><strong><?= e($theme['short_title']) ?></strong></span></a><?php endforeach; ?></nav>
    ...
    <?php if ($storageReady): ?><section class="ranking-card surface-card" data-reveal><div class="ranking-header"><div><h2><?= e(t('leaderboard.top_ten', ['theme' => $selectedTheme['short_title']])) ?></h2><span><?= e(t('leaderboard.best_only')) ?></span></div><?php if ($currentRank): ?><span class="tag teal"><i class="bi bi-person-check"></i> <?= e(t('leaderboard.your_rank', ['rank' => $currentRank])) ?></span><?php endif; ?></div>
        ...
        <div class="ranking-footer"><p><i class="bi bi-info-circle me-1"></i> <?= e(t('leaderboard.scope_note')) ?></p><a class="btn-secondary-custom btn-sm-custom" href="<?= e(challengeUrlForScoreType($selectedType)) ?>"><?= e(t($selectedTheme['mode'] === 'practical' ? 'leaderboard.try_practical' : 'leaderboard.try_quiz', ['theme' => $selectedTheme['short_title']])) ?> <i class="bi bi-arrow-right"></i></a></div>
        <?php else: ?><div class="empty-state"><div class="icon-tile amber"><i class="bi bi-trophy"></i></div><h2><?= e(t('leaderboard.be_first')) ?></h2><p><?= e(t('leaderboard.empty', ['theme' => $selectedTheme['short_title']])) ?></p><a class="btn-primary-custom" href="<?= e(challengeUrlForScoreType($selectedType)) ?>"><i class="bi bi-lightning-charge"></i> <?= e(t('leaderboard.start')) ?></a></div><?php endif; ?>
    </section><?php endif; ?>
</div></div>
