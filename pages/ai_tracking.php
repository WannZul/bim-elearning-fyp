...
<div class="page-shell"><div class="container-wide">
    ...
    <div class="ai-layout">
        <section class="camera-card surface-card" data-reveal aria-label="<?= e(t('ai.camera_label')) ?>">
            ...
            <p class="visually-hidden" id="camera-announcer" role="status" aria-live="polite" aria-atomic="true"></p>
            <p class="visually-hidden" id="detection-announcer" role="status" aria-live="polite" aria-atomic="true"></p>
        </section>
        <aside class="practice-panel">
            <section class="target-card surface-card" data-reveal><div class="target-card-header"><span><?= e(t('ai.target')) ?></span><span class="tag teal" id="practice-state"><i class="bi bi-hourglass-split" aria-hidden="true"></i> <?= e(t('ai.js.not_started')) ?></span></div><div class="target-display"><strong id="target-symbol"><?= e($initialTarget) ?></strong></div><h2 id="target-title"><?= e($initialTargetTitle) ?></h2><p id="target-instruction"><?= e(t('ai.instruction')) ?></p><div class="target-selector" role="group" aria-label="<?= e(t('ai.target_label')) ?>"><?php foreach ($allowedTargets as $target): ?><button class="target-button <?= $target === $initialTarget ? 'active' : '' ?>" type="button" data-target="<?= e($target) ?>" aria-pressed="<?= $target === $initialTarget ? 'true' : 'false' ?>"><?= e($target) ?></button><?php endforeach; ?></div></section>
...
$clientI18nKeys = [
    ...
    'ai.js.stopped', 'ai.js.not_started', 'ai.js.stability_zero', 'ai.js.title_A', 'ai.js.title_number', 'ai.js.target_changed',
];
$pageScripts = <<<'HTML'
